...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['amazon-seller', 'account-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SellerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/sellers` : null),
    () => getSellers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Seller',
      path: isTenantUser ? `/sellers/view/${data?.[0]?.id}` : '/sellers',
      entity: 'seller',
      service: AccessServiceEnum.PROJECT,
      icon: FiShoppingBag,
    },
    { name: 'Guest', path: '/guests', entity: 'guest', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
    { name: 'Keyword', path: '/keywords', entity: 'keyword', service: AccessServiceEnum.PROJECT, icon: FiSearch },
