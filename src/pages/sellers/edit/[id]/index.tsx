...
import { getSellerById, updateSellerById } from 'apiSdk/sellers';
...
function SellerEditPage() {
  ...
  const handleSubmit = async (values: SellerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSellerById(id, values);
      mutate(updated);
      resetForm();
      router.push('/sellers');
    } catch (error) {
  ...
  const formik = useFormik<SellerInterface>({
    initialValues: data,
