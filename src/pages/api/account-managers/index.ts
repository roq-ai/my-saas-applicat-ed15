...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getAccountManagers() {
    const data = await prisma.account_manager
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'account_manager'));
  ...
  async function createAccountManager() {
    ...
    const data = await prisma.account_manager.create({
      data: body,
    });
    return res.status(200).json(data);
  }
